...
- Métricas de satisfação`,
  ...
  await Promise.all(
    compositions.map(composition => prisma.composition.create({ data: composition }))
  );
  ...
  await Promise.all(
    contracts.map(contract => prisma.contract.create({ data: contract }))
  );
  ...
  await Promise.all(
    agentExecutions.map(execution => prisma.agentExecution.create({ data: execution }))
  );
  ...
  await Promise.all(
    agentMetrics.map(metric => prisma.agentMetrics.create({ data: metric }))
  );
